...
-- Garantir que as tabelas existam (com verificação)
DO $$ 
BEGIN
    -- Criar tabela design_settings se não existir
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'design_settings') THEN
        ...
        RAISE NOTICE 'Tabela design_settings criada';
    END IF;

    -- Criar tabela carousel_slides se não existir
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'carousel_slides') THEN
        ...
        RAISE NOTICE 'Tabela carousel_slides criada';
    END IF;
END $$;

-- Inserir dados padrão apenas se não existirem
INSERT INTO design_settings (
  layout, logo_url, primary_color, secondary_color, accent_color, 
  hero_title, hero_subtitle, about_text, contact_phone, contact_email, contact_address
)
SELECT 
  'modern',
...
-- Inserir slides padrão apenas se não existirem
...
DROP POLICY IF EXISTS "Allow authenticated users to insert carousel_slides" ON carousel_slides;
DROP POLICY IF EXISTS "Allow authenticated users to update carousel_slides" ON carousel_slides;
DROP POLICY IF EXISTS "Allow authenticated users to delete carousel_slides" ON carousel_slides;
...
-- Criar políticas para escrita apenas usuários autenticados
